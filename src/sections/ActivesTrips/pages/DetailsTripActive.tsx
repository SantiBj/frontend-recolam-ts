  ...
  return (
    <article className="space-y-[150px]">
      <div className="space-y-[15px]">
        ...
        <Title text="Detalles del viaje" color="white" size="lg" to="/trip-actives" />
      </div>
      <ModalGeneric
        isOpen={modal}
      ...
      <section className="bg-white p-[50px] w-[85%] md:w-[60%] max-w-[500px] mx-auto rounded-xl">
        <section className="space-y-[30px] w-fit mx-auto">
          <section className="w-fit mx-auto">
            <img className="w-[150px]" src="https://cdn-icons-png.flaticon.com/512/5639/5639164.png" alt="" />
          </section>
          <section>
            <div><span className="font-bold">Dia del viaje : </span>{dataConsult?.scheduleDay}</div>
            <div><span className="font-bold">Cliente : </span>{typeof dataConsult?.user == "object" && dataConsult?.user.name}</div>
            <div><span className="font-bold">Camion : </span>{dataConsult?.truck}</div>
            <div> <span className="font-bold">Direccion : </span>{dataConsult?.address}</div>
            <div>
              <span className="font-bold">Salida empresa : </span>{formaterDate(dataConsult?.initialDateCompany)}
            </div>
            {dataConsult?.initialDateCustomer !== null && (
              <div>
                <span className="font-bold">Llegada cliente : </span>{formaterDate(dataConsult?.initialDateCustomer)}
              </div>
            )}
            {dataConsult?.endDateCustomer !== null && (
              <div>
                <span className="font-bold">Salida cliente : </span>{formaterDate(dataConsult?.endDateCustomer)}
              </div>
            )}
          </section>
        </section>
        ...
        <section className="flex justify-center mt-[30px]">
          <button
            onClick={openModal}
            className={`bg-[#2c8d42] p-[10px] rounded-lg text-white ${((dataConsult?.endDateCustomer == null) ||
